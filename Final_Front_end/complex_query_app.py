        ...
        while i < len(input_string):
            ...
            if char not in ['+', '\\', '|', ' ', '(', ')', '~']:
                ...
                if s.strip():
                    ...
                    params.append(s)
                    params.append(s)
                    query += f" ( ISNULL(CIN, '') LIKE ? or ISNULL(LLP_Identification_Number, '') LIKE ? or ISNULL(Foreign_Company_Registration_Number, '') LIKE ? )  AND "
                    s = ""
                    if i+1<len(input_string) and input_string[i+1]==' ':
                        i+=1
            elif char == '|':
                if s.strip():
                    ...
                    params.append(s)
                    params.append(s)
                    query += f" ( ISNULL(CIN, '') LIKE ? or ISNULL(LLP_Identification_Number, '') LIKE ? or ISNULL(Foreign_Company_Registration_Number, '') LIKE ? )  OR "
                    s = ""
                    if i+1<len(input_string) and input_string[i+1]==' ':
                        i+=1
            elif char == ' ':
                ...
                if i < len(input_string):
                    ...
                    if char == '(':
                        ...
                        if s.strip():
                            ...
                            params.append(s)
                            params.append(s)
                            query += f" ( ISNULL(CIN, '') LIKE ? or ISNULL(LLP_Identification_Number, '') LIKE ? or ISNULL(Foreign_Company_Registration_Number, '') LIKE ? ) "
                            s = ""
                            query += " ) "
                            bracket_closed = True
                            if i+1<len(input_string) and input_string[i+1]==' ':
